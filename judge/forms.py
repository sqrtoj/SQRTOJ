...
class ProfileForm(ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            'site_palette': Select2Widget(attrs={'style': 'width:200px'}),
        }
...
class ProblemEditForm(ModelForm):
    ...
    def clean_code(self):
        ...
        if self.org_pk is None:
            return code
        if not getattr(settings, 'VNOJ_REQUIRE_ORG_PROBLEM_PREFIX', False):
            return code
...
class ProblemSubmitForm(ModelForm):
    source = CharField(max_length=1048576, required=False, widget=AceWidget(theme='twilight', no_ace_media=True))
