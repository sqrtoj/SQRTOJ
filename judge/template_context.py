...
def __nav_tab(path):
    ...
    return list(result[0].get_ancestors(include_self=True).values_list('key', flat=True)) if result else []


@functools.lru_cache(maxsize=1)
def _get_cached_nav_bar():
...
def general_info(request):
    ...
    return {
        ...
        'nav_bar': FixedSimpleLazyObject(_get_cached_nav_bar),
...
def get_caniuse(user_agent):
    return CanIUse(user_agent)


def math_setting(request):
    caniuse = get_caniuse(request.META.get('HTTP_USER_AGENT', ''))
