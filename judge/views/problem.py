...
from judge.models import ContestSubmission, EFFECTIVE_MATH_ENGINES, Judge, Language, Problem, ProblemGroup, \
...
class ProblemDetail(ProblemMixin, SolvedProblemMixin, CommentedDetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            ...
            context['sample_input'] = self.object.sample_input
            context['sample_output'] = self.object.sample_output
            ...
            context['sample_input'] = translation.sample_input or self.object.sample_input
            context['sample_output'] = translation.sample_output or self.object.sample_output
...
class ProblemList(QueryStringSortMixin, TitleMixin, SolvedProblemMixin, ListView):
    ...
    def get_filter(self):
        if self.in_contest:
            return Q(contests__contest=self.contest)
    ...
    def get_normal_queryset(self):
        ...
        if self.profile is not None and self.hide_solved:
            queryset = queryset.exclude(id__in=self.get_completed_problems())
        if self.show_types:
...
class ProblemEdit(ProblemMixin, TitleMixin, UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Check if AI reformat API is configured
        has_ai_api = bool(getattr(settings, 'AI_REFORMAT_API_KEY', None) or
                          getattr(settings, 'GEMINI_API_KEY', None))
        data['has_ai_reformat_api'] = has_ai_api
        return data
...
class ProblemTranslationEditForm(forms.ModelForm):
    class Meta:
...
class ProblemTranslationEdit(TitleMixin, UpdateView):
    template_name = 'problem/translate.html'
    model = ProblemTranslation
    form_class = ProblemTranslationEditForm

    def get_title(self):
        return _('Translating problem {0}').format(self.get_problem().name)

    def get_problem(self):
        return get_object_or_404(Problem, code=self.kwargs['problem'])
    ...
    def form_valid(self, form):
        ...
        # Refresh translated statement immediately instead of waiting for fragment cache expiry.
        cache.delete_many([
            make_template_fragment_key('problem_html', (translation.problem_id, engine, translation.language))
            for engine in EFFECTIVE_MATH_ENGINES
        ])
        return HttpResponseRedirect(reverse('problem_detail', args=[self.get_problem().code]))
...
def regex_reformat_description(description):
    import re

    text = description

    # Step 1: Normalize line endings and trailing whitespace
    text = text.replace('\r\n', '\n').replace('\r', '\n')
    text = re.sub(r'[ \t]+$', '', text, flags=re.MULTILINE)
    text = text.replace('\u00a0', ' ')

    # Step 1.5: Normalize basic HTML into markdown-like text
    text = re.sub(r'(?i)<br\s*/?>', '\n', text)
    text = re.sub(r'(?i)</p\s*>', '\n\n', text)
    text = re.sub(r'(?i)<p[^>]*>', '', text)
    text = re.sub(r'(?i)<strong>(.*?)</strong>', r'**\1**', text, flags=re.DOTALL)
    text = re.sub(r'(?i)<b>(.*?)</b>', r'**\1**', text, flags=re.DOTALL)
    text = re.sub(r'(?i)<em>(.*?)</em>', r'*\1*', text, flags=re.DOTALL)
    text = re.sub(r'(?i)<i>(.*?)</i>', r'*\1*', text, flags=re.DOTALL)
    text = re.sub(r'(?i)<code>(.*?)</code>', r'`\1`', text, flags=re.DOTALL)
    text = re.sub(r'(?i)<li[^>]*>\s*', '- ', text)
    text = re.sub(r'(?i)</li\s*>', '\n', text)
    text = re.sub(r'(?i)</?(ul|ol)[^>]*>', '\n', text)
    text = re.sub(r'(?i)</?(div|span)[^>]*>', '', text)

    # Step 2: Convert HTML <sup>...</sup> to ^{...} and <sub>...</sub> to _{...}
    text = re.sub(r'<sup>(.+?)</sup>', r'^{\1}', text, flags=re.IGNORECASE)
    text = re.sub(r'<sub>(.+?)</sub>', r'_{\1}', text, flags=re.IGNORECASE)

    # Step 3: Convert LaTeX display math \[...\] to $$...$$
    text = re.sub(r'\\\[(.*?)\\\]', r'$$\1$$', text, flags=re.DOTALL)

    # Step 4: Convert LaTeX inline math \(...\) to ~...~
    text = re.sub(r'\\\((.+?)\\\)', r'~\1~', text, flags=re.DOTALL)

    # Step 5: Convert $...$ to ~...~ (but NOT 237657...237657)
    text = re.sub(r'(?<!$)$(?!$)(.+?)(?<!$)$(?!$)', r'~\1~', text, flags=re.DOTALL)

    # Step 6: Fix markdown headers without space: ###Text -> ### Text
    text = re.sub(r'^(#{1,6})([^ #\n])', r'\1 \2', text, flags=re.MULTILINE)

    # Step 7: Convert inline section headers like "**Input** : content"
    _section_keywords = (
        r'Dữ liệu vào|Dữ liệu|Đầu vào|Đầu ra|Kết quả|'
        r'Giới hạn|Ràng buộc|Ví dụ|Ghi chú|Chú ý|Lưu ý|Giải thích|'
        r'Yêu cầu|Mô tả|Định dạng|'
        r'Input|Output|Example|Examples|Sample|Constraints?|'
        r'Limits?|Notes?|Hints?|Explanation|Subtask\s*\d*'
    )

    def _inline_header_replace(match):
    ...
    pat = r'\*\*(' + _section_keywords + r')\*\*\s*[:\.]?\s*(.*)'
    text = re.sub(pat, _inline_header_replace, text, flags=re.MULTILINE | re.IGNORECASE)

    # Also convert plain "Input: ..." / "Output: ..." style lines
    pat_plain = r'^\s*(' + _section_keywords + r')\s*[:\-–]\s*(.+?)\s*$'
    text = re.sub(
        pat_plain,
    ...
    # Standalone section header on its own line
    pat2 = r'^(?!\s*#)(' + _section_keywords + r')\s*[:\.]?\s*$'
    text = re.sub(pat2, r'## \1', text, flags=re.MULTILINE | re.IGNORECASE)

    # Step 8: Convert **var** to ~var~ when content looks like math
    def _is_math_content(content):
        content = content.strip()
        if not content or len(content) > 50:
            return False
        if re.match(r'^[a-zA-Z]$', content):
            return True
        if re.match(r'^[a-zA-Z0-9][a-zA-Z0-9_^{}\\]*$', content):
            common_words = {
                'input', 'output', 'example', 'examples', 'note', 'notes',
                'sample', 'constraint', 'constraints', 'hint', 'hints',
                'solution', 'answer', 'result', 'problem', 'description',
                'test', 'case', 'cases', 'data', 'format', 'task',
        ...
        if re.match(r'^\d+\s*\^\s*[{\d}]+$', content):
            return True
        if re.match(r'^[a-zA-Z0-9_^{}\\]+(\s*[+\-*/=<>]\s*[a-zA-Z0-9_^{}\\]+)+$', content):
            return True
    ...
    def _bold_to_math(match):
        content = match.group(1)
        if _is_math_content(content):
            return '~' + content + '~'
        return match.group(0)

    text = re.sub(r'(?<!\*)\*\*(?!\*)(.+?)(?<!\*)\*\*(?!\*)', _bold_to_math, text)

    # Step 9: Detect constraint expressions with ≤/≥ and wrap in ~...~
    def _wrap_constraint_line(match):
        pre = match.group(1)
        expr = match.group(2)
        post = match.group(3)
    ...
    constraint_full = (
        ...
        r'[≤≥]'                        # <=
        r'[ \t]*[\dA-Za-z_^{},.]+)'   # right side
        r'([\]\};\:\,\.\!\s]|$)'  # post group: trailing punctuation
    )
    text = re.sub(constraint_full, _wrap_constraint_line, text, flags=re.MULTILINE)

    constraint_single = (
        r'((?:^|(?<=\s)))'
        r'([a-zA-Z_]\w*[ \t]*[≤≥][ \t]*[\dA-Za-z_^{},.]+)'
        r'([\]\};\:\,\.\!\s]|$)'    # post group: trailing punctuation
    )
    text = re.sub(constraint_single, _wrap_constraint_line, text, flags=re.MULTILINE)

    # Step 10: Smarter math wrapping in plain text
    # Protected: code blocks, math, markdown links, URLs, headers
    _protected = r'(```[\s\S]*?```|~~.*?~~|\$\$.*?\$\$|~[^~]+~|\[.*?\]\(.*?\)|https?://[^\s]+|^#{1,6}\s.*$)'
    parts = re.split(_protected, text, flags=re.MULTILINE | re.DOTALL)
    ...
    for i in range(0, len(parts), 2):
        ...
        # Wrap inequality / range expressions only (avoid wrapping every single token)
        part = re.sub(
            (
                r'(?<!~)\b(\d+[ \t]*(?:<=|>=|<|>|=|≤|≥)[ \t]*'
        ...
        part = re.sub(
            r'(?<!~)\b([A-Za-z_][A-Za-z0-9_]*[ \t]*(?:<=|>=|<|>|=|≤|≥)[ \t]*[\dA-Za-z_^\.\(\)\{\},]+)\b(?!~)',
            r'~\1~',
            part,
        )

        # Wrap compact CP math tokens like a_i, x^2, n^k, i+j
        part = re.sub(r'(?<!~)\b([A-Za-z][A-Za-z0-9]*_[A-Za-z0-9]+)\b(?!~)', r'~\1~', part)
        part = re.sub(r'(?<!~)\b([A-Za-z][A-Za-z0-9]*\^[A-Za-z0-9{}]+)\b(?!~)', r'~\1~', part)
        # Deliberately avoid wrapping generic arithmetic chains (e.g., plain text "a + b")
        # because it can over-capture and corrupt constraint lines.
        parts[i] = part

    text = ''.join(parts)

    # Step 10.5: Auto-wrap sample blocks into fenced code when possible
    lines = text.split('\n')
    wrapped_lines = []
    idx = 0
    sample_heading = re.compile(
        r'^\s*##\s*(Sample\s+Input|Sample\s+Output|Ví\s*dụ\s+vào|Ví\s*dụ\s+ra)\s*$',
        re.IGNORECASE,
    )
    ...
    while idx < len(lines):
        ...
        if not sample_heading.match(line):
            idx += 1
            continue
        ...
        while j < len(lines):
            cur = lines[j]
            if section_heading.match(cur):
                break
            if cur.strip() == '' and block:
                k = j + 1
                while k < len(lines) and lines[k].strip() == '':
                    k += 1
                if k >= len(lines) or section_heading.match(lines[k]):
                    break
            block.append(cur)
            j += 1
    ...
    def _fix_math_symbols(match):
        ...
        for uc, lc in replacements:
            content = content.replace(uc, lc)
    ...
    # Step 12: Remove trailing blank lines inside code blocks (```...```)
...
@require_POST
@login_required
def basic_reformat_description(request, problem):
    logger = logging.getLogger('judge.problem')

    try:
        problem_obj = Problem.objects.get(code=problem)
    except Problem.DoesNotExist:
        return JsonResponse({'error': _('Problem not found.')}, status=404)

    if not problem_obj.is_editable_by(request.user):
        return JsonResponse({'error': _('You are not allowed to edit this problem.')}, status=403)
    ...
    if not description:
        return JsonResponse({'error': _('Description is empty.')}, status=400)

    try:
        reformatted = regex_reformat_description(description)
        problem_obj.description = reformatted
        problem_obj.save()
...
@require_POST
@login_required
def ai_reformat_description(request, problem):
    ...
    try:
        data = json.loads(request.body)
        description = data.get('description', '').strip()
        mode = data.get('mode', 'ai')  # 'basic' or 'ai'
    except (json.JSONDecodeError, AttributeError):
        return JsonResponse({'error': _('Invalid request body.')}, status=400)
    ...
    # Basic (non-AI) mode: use regex-based reformatter
    if mode == 'basic':
        try:
            reformatted = regex_reformat_description(description)
    ...
    provider = getattr(settings, 'AI_REFORMAT_PROVIDER', None)
    api_key = getattr(settings, 'AI_REFORMAT_API_KEY', None)

    # Backward compatibility: if new settings not configured, fall back to GEMINI_API_KEY
    ...
    if not provider or not api_key:
        return JsonResponse({'error': _('AI API is not configured. '
                                        'Please set AI_REFORMAT_PROVIDER and AI_REFORMAT_API_KEY '
                                        'in local_settings.py.')}, status=400)

    # Guard against accidental rapid repeated clicks/retries.
    cooldown_seconds = max(0, int(getattr(settings, 'AI_REFORMAT_USER_COOLDOWN_SECONDS', 8)))
    if cooldown_seconds > 0:
        cooldown_key = 'ai_reformat:cooldown:%s:%s:%s' % (provider, request.user.id, problem)
        if not cache.add(cooldown_key, True, timeout=cooldown_seconds):
            logger.info('AI reformat throttled for user %s on problem %s', request.user.username, problem)
            return JsonResponse({
                'error': _('You are doing this too quickly. Please wait a few seconds and try again.'),
            }, status=429)

    # Reuse result for identical content to avoid wasting Gemini quota.
    cache_ttl = max(0, int(getattr(settings, 'AI_REFORMAT_RESULT_CACHE_SECONDS', 600)))
    cache_key = None
    if cache_ttl > 0:
        text_hash = sha256(description.encode('utf-8')).hexdigest()[:32]
        cache_key = 'ai_reformat:result:%s:%s:%s' % (provider, problem, text_hash)
    ...
    try:
        if provider == 'gemini':
            reformatted = _call_gemini_api(prompt, api_key, logger, problem, request)
        elif provider == 'openai_compatible':
            api_url = getattr(settings, 'AI_REFORMAT_API_URL', None)
            model = getattr(settings, 'AI_REFORMAT_MODEL', None)
            if not api_url:
                return JsonResponse({'error': _('AI_REFORMAT_API_URL is not configured.')}, status=400)
            if not model:
                return JsonResponse({'error': _('AI_REFORMAT_MODEL is not configured.')}, status=400)
            reformatted = _call_openai_compatible_api(prompt, api_url, api_key, model, logger, problem, request)
        else:
            return JsonResponse({'error': _('Unknown AI provider: %s') % provider}, status=400)

        if cache_key and cache_ttl > 0 and reformatted:
            cache.set(cache_key, reformatted, timeout=cache_ttl)

        return JsonResponse({'description': reformatted})

    except _AIReformatError as e:
        # If provider is throttled (429), degrade gracefully to regex formatter.
        if e.status_code == 429 and mode == 'ai':
            logger.warning('AI reformat fallback to regex for problem %s by user %s',
                           problem, request.user.username)
            try:
                reformatted = regex_reformat_description(description)
                return JsonResponse({
...
class _AIReformatError(Exception):
    """Custom exception for AI reformat errors with HTTP status code."""
    def __init__(self, message, status_code=400):
        super().__init__(message)
        self.status_code = status_code


def _call_gemini_api(prompt, api_key, logger, problem, request):
    import requests as http_requests

    model = getattr(settings, 'AI_REFORMAT_GEMINI_MODEL', 'gemini-2.0-flash')
    url = f'https://generativelanguage.googleapis.com/v1beta/models/{model}:generateContent?key={api_key}'
    max_retries = max(0, int(getattr(settings, 'AI_REFORMAT_GEMINI_MAX_RETRIES', 2)))
    base_backoff = max(1, int(getattr(settings, 'AI_REFORMAT_GEMINI_BACKOFF_SECONDS', 2)))
    response = None
    ...
    for attempt in range(max_retries + 1):
        ...
        if attempt >= max_retries:
            ...
            try:
                ...
                google_error_message = payload.get('error', {}).get('message', '') or ''
            except Exception:
                google_error_message = ''

            logger.warning('Gemini API rate limit exceeded for problem %s by user %s after %d attempts. detail=%s',
                           problem, request.user.username, attempt + 1, google_error_message[:300])
            raise _AIReformatError(
                _('Gemini API rate limit exceeded. Please wait a moment and try again.'),
                status_code=429,
            )

        retry_after = response.headers.get('Retry-After')
        try:
            sleep_seconds = int(retry_after) if retry_after else (base_backoff * (2 ** attempt))
        except (TypeError, ValueError):
            sleep_seconds = base_backoff * (2 ** attempt)

        sleep_seconds = max(1, min(sleep_seconds, 20))
        logger.info('Gemini returned 429 for problem %s, retrying in %ss (attempt %d/%d)',
                    problem, sleep_seconds, attempt + 1, max_retries + 1)
        time.sleep(sleep_seconds)

    if response.status_code != 200:
        logger.error('Gemini API error (status %d) for problem %s: %s',
                     response.status_code, problem, response.text[:500])
        raise _AIReformatError(
            _('Gemini API error (status %d): %s') % (response.status_code, response.text[:200]),
        )

    result = response.json()
    candidates = result.get('candidates', [])
    if not candidates:
        logger.warning('Gemini API returned no candidates for problem %s', problem)
        raise _AIReformatError(_('Gemini API returned no results.'))

    reformatted = candidates[0].get('content', {}).get('parts', [{}])[0].get('text', '')
    if not reformatted:
        raise _AIReformatError(_('Gemini API returned empty result.'))

    return reformatted


def _call_openai_compatible_api(prompt, api_url, api_key, model, logger, problem, request):
    import requests as http_requests
    ...
    payload = {
        'model': model,
        'messages': [
            {'role': 'system', 'content': 'You are a competitive programming problem formatter.'},
    ...
    response = http_requests.post(api_url, json=payload, headers=headers, timeout=60)

    if response.status_code == 429:
        logger.warning('AI API rate limit exceeded for problem %s by user %s',
                       problem, request.user.username)
        raise _AIReformatError(
            _('AI API rate limit exceeded. Please wait a moment and try again.'),
            status_code=429,
        )

    if response.status_code != 200:
        logger.error('AI API error (status %d) for problem %s: %s',
                     response.status_code, problem, response.text[:500])
        raise _AIReformatError(
            _('AI API error (status %d): %s') % (response.status_code, response.text[:200]),
        )

    result = response.json()
    choices = result.get('choices', [])
    if not choices:
        logger.warning('AI API returned no choices for problem %s', problem)
        raise _AIReformatError(_('AI API returned no results.'))

    reformatted = choices[0].get('message', {}).get('content', '')
    if not reformatted:
        raise _AIReformatError(_('AI API returned empty result.'))
