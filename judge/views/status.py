...
def version_matrix(request):
    ...
    for data in matrix.values():
        for language, versions in data.items():
            versions.versions = [version_key(runtime.version) for runtime in versions]
    ...
    languages = sorted(languages, key=lambda lang: (lang.name or '').casefold())
