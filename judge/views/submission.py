...
def submission_related(queryset):
    return queryset.select_related('user__user', 'user__display_badge', 'problem', 'language') \
        .only('id', 'user__user__username', 'user__display_rank', 'user__rating', 'problem__name', 'problem__code',
              ...
              'contest_object__key', 'contest_object__name', 'problem__suggester_id') \
...
def _get_cached_visible_problem_ids(user_id, time_block):
    q = Q(is_public=True, is_organization_private=False)
    if user_id is not None:
        q = q | Q(authors__user_id=user_id) | Q(curators__user_id=user_id) | Q(testers__user_id=user_id)
    return list(Problem.objects.filter(q).values_list('id', flat=True))


@functools.lru_cache(maxsize=256)
def _get_cached_visible_contest_ids(user_id, time_block):
    contest_filter = Q(scoreboard_visibility=Contest.SCOREBOARD_VISIBLE) | Q(end_time__lt=timezone.now())
    if user_id is not None:
        contest_filter |= Q(authors__user_id=user_id) | Q(curators__user_id=user_id)
    return list(Contest.objects.filter(contest_filter).values_list('id', flat=True))


def _get_visible_problem_filter(user):
    """Return a Q filter for visible problems, used in the fast two-step query."""
    if user.is_superuser:
        return Q()
    time_block = int(time.time()) // 300
    user_id = user.id if user.is_authenticated else None
    ids = _get_cached_visible_problem_ids(user_id, time_block)
    return Q(problem_id__in=ids)
...
class SubmissionsListBase(DiggPaginatorMixin, TitleMixin, ListView):
    ...
    def _get_result_data(self, queryset=None):
        if queryset is None:
            # Use the base ID queryset (not the values_list one) for proper aggregation
            queryset = self._get_base_id_queryset()
    ...
    # _get_id_queryset removed — use _get_base_id_queryset instead (was duplicate code)
    # _get_queryset removed — dead code after two-step pagination refactor

    def _get_base_id_queryset(self):
        """Get a lightweight queryset for fetching submission IDs with minimal JOINs."""
        ...
        if self.in_contest:
            ...
            queryset = queryset.filter(_get_visible_problem_filter(self.request.user))

            if not self.request.user.has_perm('judge.see_private_contest'):
                time_block = int(time.time()) // 300
                user_id = self.request.user.id if self.request.user.is_authenticated else None
                visible_contest_ids = _get_cached_visible_contest_ids(user_id, time_block)
                if self.request.user.is_authenticated:
                    queryset = queryset.filter(Q(user=self.request.profile) |
                                               Q(contest_object_id__in=visible_contest_ids) |
                                               Q(contest_object__isnull=True))
                else:
                    queryset = queryset.filter(Q(contest_object_id__in=visible_contest_ids) |
                                               Q(contest_object__isnull=True))
    ...
    def get_queryset(self):
        # Return the lightweight ID queryset; actual data fetching happens in paginate_queryset.
        return self._get_base_id_queryset().values_list('id', flat=True)

    def _fetch_full_submissions(self, ids):
        """Fetch full submission data for a list of IDs, maintaining order."""
        ...
        if not self.in_contest:
            queryset = queryset.select_related('contest_object') \
                .defer('contest_object__description', 'contest_object__summary',
                       'contest_object__access_code', 'contest_object__format_config',
                       'contest_object__problem_label_script', 'contest_object__csv_ranking',
                       'contest_object__ranking_access_code')

        objects = {obj.id: obj for obj in queryset}
        return [objects[i] for i in ids if i in objects]
    ...
    def paginate_queryset(self, queryset, page_size):
        """Override to implement two-step pagination for submissions.

        Step 1: Get submission IDs from the lightweight queryset (fast, uses PRIMARY KEY index).
        Step 2: Fetch full related data for only those IDs.
        """
        paginator, page, object_list, has_other = super().paginate_queryset(queryset, page_size)

        # object_list contains lightweight submission objects (only IDs).
        # Now fetch full data with all relations for these specific IDs.
        ids = [obj.id if hasattr(obj, 'id') else obj for obj in object_list]
        if ids:
            full_submissions = self._fetch_full_submissions(ids)
            # Replace the object_list with fully-loaded submissions
            page.object_list = full_submissions
            object_list = full_submissions
...
class AllSubmissions(InfinitePaginationMixin, SubmissionsListBase):
    ...
    def paginate_queryset(self, queryset, page_size):
        paginator, page, object_list, has_other = super().paginate_queryset(queryset, page_size)
        # InfinitePaginationMixin bypasses SubmissionsListBase.paginate_queryset
        # when in infinite mode, so object_list may contain raw IDs (integers)
        # instead of full Submission objects. Convert them here.
        if object_list and not isinstance(object_list[0], Submission):
            ids = list(object_list)
            if ids:
                full_submissions = self._fetch_full_submissions(ids)
                page.object_list = full_submissions
                object_list = full_submissions
...
class ForceContestMixin(object):
    ...
    def access_check(self, request):
        ...
        if request.user.is_authenticated and (
            request.user.has_perm('judge.see_private_contest') or
            request.user.has_perm('judge.edit_all_contest') or
            request.profile.id in self.contest.editor_ids or
            request.profile.id in self.contest.tester_ids or
            self.contest.view_contest_scoreboard.filter(id=request.profile.id).exists()
        ):
            return
