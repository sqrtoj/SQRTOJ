...
class UserAboutPage(UserPage):
    ...
    def get_context_data(self, **kwargs):
        ...
        cache_key = 'user_submissions_graph:%s:%s' % (self.object.id, timezone_offset)
        submission_chart_data = cache.get(cache_key)
        if submission_chart_data is None:
            submissions = (
        ...
        context['submission_data'] = mark_safe(json.dumps(submission_chart_data['data']))
        context['submission_metadata'] = mark_safe(json.dumps({
            'min_year': submission_chart_data['min_year'],
        }))
        return context
