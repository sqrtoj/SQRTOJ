...
from judge.models import Comment, CommentVote, Profile
...
def comment_hide(request):
    ...
    subtree = comment.get_descendants(include_self=True)
    author_ids = list(subtree.values_list('author_id', flat=True).distinct())

    with transaction.atomic():
        subtree.update(hidden=True)

    for profile in Profile.objects.filter(id__in=author_ids).only('id', 'contribution_points'):
