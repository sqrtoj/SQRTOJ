    ...
    def get_contest_json(self):
        ...
        for participation in self.contest.users.filter(virtual=0):
            ...
            users[str(cnt_user)] = {
                ...
                "name": participation.user.user.first_name or participation.user.user.username,
            ...
            for order in range(1, num_problems + 1):
                for subtask in range(1, problem_sub[order - 1] + 1):
                    ...
                    problems_json[str(order)][str(subtask)] = round(
                        total_subtask_points_map.get((order, subtask), 0) /
                        total_points[order] * problem_points_map[order],
                        self.contest.points_precision,
                    )
                    users[str(cnt_user)]["problems"][str(order)]["points"][str(subtask)] = round(
                        points_map.get((order, subtask), 0) / total_points[order] * problem_points_map[order],
                        self.contest.points_precision,
                    )
                    users[str(cnt_user)]["problems"][str(order)]["frozen_points"][str(subtask)] = round(
                        frozen_points_map.get((order, subtask), 0) / total_points[order] * problem_points_map[order],
                        self.contest.points_precision,
                    )
